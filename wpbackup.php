...
Plugin URI: http://www.wordpressconnect.net/wordpress-database-backup-plugin/
Description: On-demand backup of your WordPress database. Navigate to <a href="edit.php?page=wpbackup">Tools &rarr; Backup</a> to get started.
Author: Jason Jones 
Author URI: http://www.wordpressconnect.net/wordpress-database-backup-plugin/
Version: 7.9.1

Copyright 2010  Jason Jones  (email : Jason at wordpressconnect.net)
...
class wpdbBackup {
		...
		if (isset($_GET['backup'])) {
			...
			case 'email':
				...
				if ( $success ) {
					...
					';
					echo '
						alert("' . __('Backup Complete!','wpbackup') . '");
	...
	function init_textdomain() {
		load_plugin_textdomain('wpbackup', str_replace(ABSPATH, '', dirname(__FILE__)), dirname(plugin_basename(__FILE__)));
	}
	...
	function update_notice_action() {
		...
		function update_notice($text = '') {
			...
			$replace = '$1' . "\n<p>" . sprintf(__('Click <a href="%s" target="_blank">here</a> to back up your database using the WordPress Database Backup plugin. <strong>Note:</strong> WordPress Database Backup does <em>not</em> back up your files, just your database.', 'wpbackup'), 'tools.php?page=wpbackup') . "</p>\n"; 
	...
	function build_backup_script() {
		...
		echo "<div class='wrap'>";
		echo 	'<fieldset class="options"><legend>' . __('Progress','wpbackup') . '</legend>
			<p><strong>' .
				__('DO NOT DO THE FOLLOWING AS IT WILL CAUSE YOUR BACKUP TO FAIL:','wpbackup').
			'</strong></p>
			<ol>
				<li>'.__('Close this browser','wpbackup').'</li>
				<li>'.__('Reload this page','wpbackup').'</li>
				<li>'.__('Click the Stop or Back buttons in your browser','wpbackup').'</li>
			</ol>
			<p><strong>' . __('Progress:','wpbackup') . '</strong></p>
			...
			window.onbeforeunload = function() {
				return "' . __('Navigating away from this page will cause your backup to fail.', 'wpbackup') . '";
			}
		...
		case 'http':
			echo '
				setProgress("' . __('Backup Complete!','wpbackup') . '");
				window.onbeforeunload = null; 
		...
		case 'smtp':
			...
			echo '
				setProgress("' . sprintf(__('Your backup has been emailed to %s','wpbackup'), $_POST['backup_recipient']) . '");
				window.onbeforeunload = null; 
	...
	function backup_fragment($table, $segment, $filename) {
		...
		if($table == '') {
			$msg = __('Creating backup file...','wpbackup');
		} else {
			if($segment == -1) {
				$msg = sprintf(__('Finished backing up table \\"%s\\".','wpbackup'), $table);
			} else {
				$msg = sprintf(__('Backing up table \\"%s\\"...','wpbackup'), $table);
			}
		}
		...
		if (is_writable($this->backup_dir)) {
			...
			if(!$this->fp) {
				$this->error(__('Could not open the backup file for writing!','wpbackup'));
				$this->error(array('loc' => 'frame', 'kind' => 'fatal', 'msg' =>  __('The backup file could not be saved.  Please check the permissions for writing to your backup directory and try again.','wpbackup')));
			}
			else {
				if($table == '') {		
					//Begin new backup of MySql
					$this->stow("# " . __('WordPress MySQL database backup','wpbackup') . "\n");
					$this->stow("#\n");
					$this->stow("# " . sprintf(__('Generated: %s','wpbackup'),date("l j. F Y H:i T")) . "\n");
					$this->stow("# " . sprintf(__('Hostname: %s','wpbackup'),DB_HOST) . "\n");
					$this->stow("# " . sprintf(__('Database: %s','wpbackup'),$this->backquote(DB_NAME)) . "\n");
					$this->stow("# --------------------------------------------------------\n");
				} else {
					if($segment == 0) {
						...
						$this->stow("# --------------------------------------------------------\n");
						$this->stow("# " . sprintf(__('Table: %s','wpbackup'),$this->backquote($table)) . "\n");
						$this->stow("# --------------------------------------------------------\n");
					}			
			...
			$this->error(array('kind' => 'fatal', 'loc' => 'frame', 'msg' => __('The backup directory is not writeable!  Please check the permissions for writing to your backup directory and try again.','wpbackup')));
		}
			...
			addLoadEvent(function() {
				...
				for ( var k in t ) {
					...
					if ( ul ) {
						...
						if ( 2 < lis.length ) {
							...
							text.innerHTML = '<?php _e('Click and hold down <code>[SHIFT]</code> to toggle multiple checkboxes', 'wpbackup'); ?>';
				...
				var initTimeChange = function() {
					...
					?> ) ) {
						...
						span.id = 'change-wrap';
						span.innerHTML = '<input type="submit" id="change-backup-time" name="change-backup-time" value="<?php _e('Change','wpbackup'); ?>" />';
				...
				var clickTime = function(e, backupTime) {
					...
					backupTime.innerHTML = '<input type="text" value="' + tText + '" name="backup-time-text" id="backup-time-text" /> <span class="submit"><input type="submit" name="save-backup-time" id="save-backup-time" value="<?php _e('Save', 'wpbackup'); ?>" /></span>';
	...
	function admin_menu() {
		$_page_hook = add_management_page(__('Backup','wpbackup'), __('Backup','wpbackup'), 'import', $this->basename, array(&$this, 'backup_menu'));
	...
	function fragment_menu() {
		$page_hook = add_management_page(__('Backup','wpbackup'), __('Backup','wpbackup'), 'import', $this->basename, array(&$this, 'build_backup_script'));
		add_action('load-' . $page_hook, array(&$this, 'admin_load'));
	}

	/** 
	 * Add wpbackup-specific help options to the 2.7 =< WP contextual help menu
	 * return string The text of the help menu.
	 */
	function help_menu() {
		$text = "\n<a href=\"http://wordpress.org/extend/plugins/wpbackup/faq/\" target=\"_blank\">" . __('FAQ', 'wpbackup') . '</a>';
		$text .= "\n<br />\n<a href=\"http://www.ilfilosofo.com/forum/forum/2\" target=\"_blank\">" . __('wpbackup Support Forum', 'wpbackup') . '</a>';
		return $text;
	}
	...
	function stow($query_line) {
		if(false === @fwrite($this->fp, $query_line))
			$this->error(__('There was an error writing a line to the backup script:','wpbackup') . '  ' . $query_line . '  ' . $php_errormsg);
	}
	...
	function error_display($loc = 'main', $echo = true) {
		...
		if ( 10 == count( $err_list ) )
			$err_list[9] = __('Subsequent errors have been omitted from this log.','wpbackup');
	...
	function backup_table($table, $segment = 'none') {
		...
		if (! $table_structure) {
			$this->error(__('Error getting table details','wpbackup') . ": $table");
			return false;
		}
		...
		if(($segment == 'none') || ($segment == 0)) {
			...
			$this->stow("\n\n");
			$this->stow("#\n");
			$this->stow("# " . sprintf(__('Delete any existing table %s','wpbackup'),$this->backquote($table)) . "\n");
			$this->stow("#\n");
			$this->stow("\n");
			...
			$this->stow("\n\n");
			$this->stow("#\n");
			$this->stow("# " . sprintf(__('Table structure of table %s','wpbackup'),$this->backquote($table)) . "\n");
			$this->stow("#\n");
			$this->stow("\n");
			
			$create_table = $wpdb->get_results("SHOW CREATE TABLE $table", ARRAY_N);
			if (false === $create_table) {
				$err_msg = sprintf(__('Error with SHOW CREATE TABLE for %s.','wpbackup'), $table);
				$this->error($err_msg);
				$this->stow("#\n# $err_msg\n#\n");
			}
			...
			if (false === $table_structure) {
				$err_msg = sprintf(__('Error getting table structure of %s','wpbackup'), $table);
				$this->error($err_msg);
				$this->stow("#\n# $err_msg\n#\n");
			}
			...
			$this->stow("\n\n");
			$this->stow("#\n");
			$this->stow('# ' . sprintf(__('Data contents of table %s','wpbackup'),$this->backquote($table)) . "\n");
			$this->stow("#\n");
		}
		...
		if(($segment == 'none') || ($segment < 0)) {
			...
			$this->stow("\n");
			$this->stow("#\n");
			$this->stow("# " . sprintf(__('End of data contents of table %s','wpbackup'),$this->backquote($table)) . "\n");
			$this->stow("# --------------------------------------------------------\n");
			$this->stow("\n");
		}
	} // end backup_table()
	...
	function db_backup($core_tables, $other_tables) {
		...
		if (is_writable($this->backup_dir)) {
			...
			$this->error(__('The backup directory is not writeable!','wpbackup'));
			return false;
		}
	...
	function deliver_backup($filename = '', $delivery = 'http', $recipient = '', $location = 'main') {
		...
		if ('http' == $delivery) {
			if (! file_exists($diskfile)) 
				$this->error(array('kind' => 'fatal', 'msg' => sprintf(__('File not found:%s','wpbackup'), "&nbsp;<strong>$filename</strong><br />") . '<br /><a href="' . $this->page_url . '">' . __('Return to Backup','wpbackup') . '</a>'));
			...
			if (! file_exists($diskfile)) {
				$msg = sprintf(__('File %s does not exist!','wpbackup'), $diskfile);
				$this->error($msg);
				return false;
			}
			...
			$message = sprintf(__("Attached to this email is\n   %1s\n   Size:%2s kilobytes\n",'wpbackup'), $filename, round(filesize($diskfile)/1024));
			$success = $this->send_mail($recipient, get_bloginfo('name') . ' ' . __('Database Backup','wpbackup'), $message, $diskfile);

			if ( false === $success ) {
				$msg = __('The following errors were reported:','wpbackup') . "\n ";
				if ( function_exists('error_get_last') ) {
					$err = error_get_last();
					$msg .= $err['message'];
				} else {
					$msg .= __('ERROR: The mail application has failed to deliver the backup.','wpbackup'); 
				}
	...
	function backup_menu() {
		...
		if ( $this->backup_complete ) {
			$feedback = '<div class="updated wpbackup-updated"><p>' . __('Backup Successful','wpbackup') . '!';
			$file = $this->backup_file;
			switch($_POST['deliver']) {
			case 'http':
				$feedback .= '<br />' . sprintf(__('Your backup file: <a href="%1s">%2s</a> should begin downloading shortly.','wpbackup'), WP_BACKUP_URL . "{$this->backup_file}", $this->backup_file);
				break;
			case 'smtp':
				...
				$feedback = '<br />' . sprintf(__('Your backup has been emailed to %s','wpbackup'), $feedback);
				break;
			case 'none':
				$feedback .= '<br />' . __('Your backup file has been saved on the server. If you would like to download it now, right click and select "Save As"','wpbackup');
				$feedback .= ':<br /> <a href="' . WP_BACKUP_URL . "$file\">$file</a> : " . sprintf(__('%s bytes','wpbackup'), filesize($this->backup_dir . $file));
			}
			$feedback .= '</p></div>';
		}
		...
		if (count($this->errors)) {
			$feedback .= '<div class="updated wpbackup-updated error"><p><strong>' . __('The following errors were reported:','wpbackup') . '</strong></p>';
		...
		if ( (function_exists('wp_schedule_event') || function_exists('wp_cron_init')) 
			...
			$feedback .= '<div class="updated wpbackup-updated"><p>' . __('Scheduled Backup Options Saved!','wpbackup') . '</p></div>';
		endif;
...
//		$dir_perms = $stat['mode'] & 0000777; // Get the permission bits.
		...
		if ( ! file_exists($this->backup_dir) && ! @mkdir($this->backup_dir) ) {
			?><div class="updated wpbackup-updated error"><p><?php _e('WARNING: Your backup directory does <strong>NOT</strong> exist, and we cannot create it.','wpbackup'); ?></p>
			<p><?php printf(__('Using your FTP client, try to create the backup directory yourself: %s', 'wpbackup'), '<code>' . $this->backup_dir . '</code>'); ?></p></div><?php
			$whoops = true;
		// not writable due to write permissions
		} elseif ( !is_writable($this->backup_dir) && ! @chmod($this->backup_dir, $dir_perms) ) {
			?><div class="updated wpbackup-updated error"><p><?php _e('WARNING: Your backup directory is <strong>NOT</strong> writable! We cannot create the backup files.','wpbackup'); ?></p>
			<p><?php printf(__('Using your FTP client, try to set the backup directory&rsquo;s write permission to %1$s or %2$s: %3$s', 'wpbackup'), '<code>777</code>', '<code>a+w</code>', '<code>' . $this->backup_dir . '</code>'); ?>
			</p></div><?php 
			$whoops = true;
		} else {
			...
			// the directory is not writable probably due to safe mode
				...
				?><div class="updated wpbackup-updated error"><p><?php _e('WARNING: Your backup directory is <strong>NOT</strong> writable! We cannot create the backup files.','wpbackup'); ?></p><?php 
				if( ini_get('safe_mode') ){
					?><p><?php _e('This problem seems to be caused by your server&rsquo;s <code>safe_mode</code> file ownership restrictions, which limit what files web applications like WordPress can create.', 'wpbackup'); ?></p><?php 
				}
				?><?php printf(__('You can try to correct this problem by using your FTP client to delete and then re-create the backup directory: %s', 'wpbackup'), '<code>' . $this->backup_dir . '</code>');
				?></div><?php 
				$whoops = true;
			}
		}
		...
		?><div class='wrap'>
		<h2><?php _e('Backup','wpbackup') ?></h2>
		<form method="post" action="">
		<?php if ( function_exists('wp_nonce_field') ) wp_nonce_field($this->referer_check_key); ?>
		<fieldset class="options"><legend><?php _e('Tables','wpbackup') ?></legend>
		<div class="tables-list core-tables alternate">
		<h4><?php _e('These core WordPress tables will always be backed up:','wpbackup') ?></h4><ul><?php
		$excs = (array) get_option('wp_db_backup_excs');
		foreach ($wp_backup_default_tables as $table) {
			if ( $table == $wpdb->comments ) {
				...
				echo "<li><input type='hidden' name='core_tables[]' value='$table' /><code>$table</code> <span class='instructions'> <input type='checkbox' name='exclude-spam[]' value='$table' $checked /> " . __('Exclude spam comments', 'wpbackup') . '</span></li>';
				...
				echo "<li><input type='hidden' name='core_tables[]' value='$table' /><code>$table</code> <span class='instructions'> <input type='checkbox' name='exclude-revisions[]' value='$table' $checked /> " . __('Exclude post revisions', 'wpbackup') . '</span></li>';
			} else {
		...
		if (count($other_tables) > 0) { 
			?>
			<h4><?php _e('You may choose to include any of the following tables:','wpbackup'); ?></h4>
			<ul>
			<?php
		...
		<fieldset class="options">
			<legend><?php _e('Backup Options','wpbackup'); ?></legend>
			<p><?php  _e('What to do with the backup file:','wpbackup'); ?></p>
			<ul>
			<li><label for="do_save">
				<input type="radio" id="do_save" name="deliver" value="none" style="border:none;" />
				<?php _e('Save to server','wpbackup'); 
			...
			<p class="submit">
				<input type="submit" name="submit" onclick="document.getElementById('do_backup').value='fragments';" value="<?php _e('Backup now!','wpbackup'); ?>" />
			</p>
			<?php else : ?>
				<div class="updated wpbackup-updated error"><p><?php _e('WARNING: Your backup directory is <strong>NOT</strong> writable!','wpbackup'); ?></p></div>
			<?php endif; // ! whoops ?>
		</fieldset>
		...
		if ( $cron_old || $cron ) :
			echo '<fieldset class="options"><legend>' . __('Scheduled Backup','wpbackup') . '</legend>';
			...
			if ( $cron ) :
				...
				if ( ! empty( $next_cron ) ) :
					...
					<?php printf(__('Next Backup: %s','wpbackup'), '<span id="next-backup-time">' . gmdate($datetime, $next_cron + (get_option('gmt_offset') * 3600)) . '</span>'); ?>
					</p>
					<?php 
				endif;
			elseif ( $cron_old ) :
				?><p><?php printf(__('Last WP-Cron Daily Execution: %s','wpbackup'), gmdate($datetime, get_option('wp_cron_daily_lastrun') + (get_option('gmt_offset') * 3600))); ?><br /><?php 
				printf(__('Next WP-Cron Daily Execution: %s','wpbackup'), gmdate($datetime, (get_option('wp_cron_daily_lastrun') + (get_option('gmt_offset') * 3600) + 86400))); ?></p><?php 
			endif;
			?><form method="post" action="">
			<?php if ( function_exists('wp_nonce_field') ) wp_nonce_field($this->referer_check_key); ?>
			<div class="tables-list">
			<h4><?php _e('Schedule: ','wpbackup'); ?></h4>
			<?php 
			if ( $cron_old ) :
				$wp_cron_backup_schedule = get_option('wp_cron_backup_schedule');
				$schedule = array(0 => __('None','wpbackup'), 1 => __('Daily','wpbackup'));
			...
			$cron_recipient_input = '<p><label for="cron_backup_recipient">' . __('Email backup to:','wpbackup') . ' <input type="text" name="cron_backup_recipient" id="cron_backup_recipient" size="20" value="' . $cron_recipient . '" /></label></p>';
			echo apply_filters('wp_db_b_cron_recipient_input', $cron_recipient_input);
			echo '<p class="submit"><input type="submit" name="submit" value="' . __('Schedule backup','wpbackup') . '" /></p>';
			echo '</div>';
			...
			if (count($other_tables) > 0) {
				...
				echo '<h4>' . __('Tables to include in the scheduled backup:','wpbackup') . '</h4><ul>';
				foreach ($other_tables as $table) {
	...
	function schedule_choices($schedule) { // create the cron menu based on the schedule
		...
		$menu = '<ul>';
		$schedule = array_merge( array( 'never' => array( 'interval' => 0, 'display' => __('Never','wpbackup') ) ),
			(array) $schedule_sorted );
		foreach ( $schedule as $name => $settings) {
			$interval = (int) $settings['interval'];
			if ( 0 == $interval && ! 'never' == $name ) continue;
			$display = ( ! '' == $settings['display'] ) ? $settings['display'] : sprintf(__('%s seconds','wpbackup'),$interval);
	...
	function add_sched_options($sched) {
		$sched['weekly'] = array('interval' => 604800, 'display' => __('Once Weekly','wpbackup'));
		return $sched;
	}
	...
	function can_user_backup($loc = 'main') {
		...
		if ( false == $can ) 
			$this->error(array('loc' => $loc, 'kind' => 'fatal', 'msg' => __('You are not allowed to perform backups.','wpbackup')));
		return $can;
	}
	...
	function verify_nonce($rec = '', $nonce = 'X', $loc = 'main') {
		if ( wp_verify_nonce($rec, $nonce) )
			...
			$this->error(array('loc' => $loc, 'kind' => 'fatal', 'msg' => sprintf(__('There appears to be an unauthorized attempt from this site to access your database located at %1s.  The attempt has been halted.','wpbackup'),get_option('home'))));
	}
	...
	function validate_file($file) {
		if ( (false !== strpos($file, '..')) || (false !== strpos($file, './')) || (':' == substr($file, 1, 1)) )
			$this->error(array('kind' => 'fatal', 'loc' => 'frame', 'msg' => __("Cheatin' uh ?",'wpbackup')));
	}
